...
# bf16 only, unsupport fp16
# cuda operator support is required
import tqdm
...
def train(audio_model, train_loader, test_loader, mixup_fn, re_fn,  args, max_norm: float = None):
    ...
    optimizer = torch.optim.AdamW(trainables, args.lr, weight_decay=1e-4, betas=(0.9, 0.95))
    ...
    print('now training with {:s}, main metrics: {:s}, loss function: {:s}'.format(str(args.dataset), str(main_metrics), str(loss_fn)))
    ...
    # stats, valid_loss = validate(audio_model, test_loader, args, epoch, writer)
    while epoch < args.epochs + 1:
        pbar = tqdm.tqdm(train_loader, desc=f"Epoch {epoch}", unit="batch")
        ...
        for i, (audio_input, labels) in enumerate(BackgroundGenerator(pbar, max_prefetch=8)):

            audio_input = audio_input.bfloat16().to(device, non_blocking=True).unsqueeze(1)
            labels = labels.to(device, non_blocking=True)
            if i % accum_iter == 0:
                lr = adjust_learning_rate(optimizer, i / len(train_loader) + epoch, args)
            if re_fn != None:
                audio_input = re_fn(audio_input)
            if mixup_fn != None:
                audio_input, labels = mixup_fn(audio_input, labels)
            ...
            with autocast(enabled=False):
                ...
                # else:
                    # print(audio_output.shape ,labels.shape)
                loss = loss_fn(audio_output, labels.bfloat16())
            ...
            #             parameters=audio_model.parameters(), create_graph=False,
            ...
            # record loss
            loss_meter.update(loss_value, B)
            ...
            'lr': f'{lr:.6e}'
            # 'btime': f'{batch_time.avg:.3f}s'
            # 'Total Time': f'{per_sample_time.avg:.5f}',
            # 'Data Time': f'{per_sample_data_time.avg:.5f}',
            # 'DNN Time': f'{per_sample_dnn_time.avg:.5f}'
            })
            ...
            #       'Train Loss {loss_meter.avg:.6f}\t'.format(
        ...
        writer.add_scalar('Val/AUC',   mAUC,   epoch)
...
def validate(audio_model, val_loader, args, epoch, writer):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # batch_time = AverageMeter()
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
    ...
    with torch.no_grad():
        for i, (audio_input, labels) in enumerate(val_loader):
            ...
            # compute output
            with autocast(enabled=False):
                audio_output = audio_model(audio_input.bfloat16())
            audio_output = torch.sigmoid(audio_output.float())
            ...
            if isinstance(args.loss_fn, torch.nn.CrossEntropyLoss):
                ...
                loss = loss_fn(audio_output, labels.bfloat16())
            A_loss.append(loss.float().to('cpu').detach())
